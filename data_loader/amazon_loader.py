...
class SCRecDset(Dataset):
    def __init__(self, data_path, data_name, train_mode='train', tokenizer=None, templates=None, ceid_dict='', seid_dict='', num_templates=10, test_instruction_type='ceid', seed=2025, is_p5id=False):
        ...
        if type(self.ceid_dict) is list:
            self.ceid_dict = self.ceid_dict[0]
        self.reverse_ceid = {tuple(v): k for k, v in self.ceid_dict.items()}

        self.seid_dict = self.load_json(os.path.join(data_path, data_name, seid_dict))
        if type(self.seid_dict) is list:
            self.seid_dict = self.seid_dict[0]
        self.reverse_seid = {tuple(v): k for k, v in self.seid_dict.items()}

        self.iid_ceid, self.iid_seid, self.ceid_iid, self.seid_iid, self.idx_iid = self.index_to_input_dict(self.ceid_dict, self.seid_dict, is_p5id)
        ...
        self.ceid_tree = SampleTree()
        for key, path in self.ceid_dict.items():
            self.ceid_tree.insert(path)
        self.seid_tree = SampleTree()
        for key, path in self.seid_dict.items():
            self.seid_tree.insert(path)

    def index_to_input_dict(self, ceid_dict, seid_dict, is_p5id=False):
    ...
    def __len__(self):
        if self.train_mode == 'train':
            # return self.total_num * len(self.templates) * len(self.task_instruction)
    ...
    def __getitem__(self, idx):
        out_dict = {}
        if self.train_mode == "train":
            # instruction_idx = idx // (self.total_num * len(self.templates))
            # instruction = self.task_instruction[instruction_idx]
            instruction = random.choice(self.task_instruction)
            # template_idx = idx // self.total_num % len(self.templates)
        ...
        if pred_type == '<C>':
            ...
            neg_sample_ceid = self.reverse_ceid[tuple(self.ceid_tree.sampling_negative(self.ceid_dict[target_item]))]
            neg_sample_sent = convert_iid[neg_sample_ceid]
        else:
            convert_iid = self.iid_seid
            neg_sample_seid = self.reverse_seid[tuple(self.seid_tree.sampling_negative(self.seid_dict[target_item]))]
            neg_sample_sent = convert_iid[neg_sample_seid]
        ...
        out_dict['neg_ids'] = torch.LongTensor(neg_id)
        out_dict['neg_sent'] = neg_sample_sent
        ...
        out_dict['neg_length'] = len(neg_id)
        return out_dict
    ...
    def collate_fn(self, batch):
        ...
        max_neg_length = max(entry['neg_length'] for entry in batch)
        ...
        neg_ids = torch.ones(B, max_neg_length, dtype=torch.long) * -100
        neg_length = torch.zeros(B, 1, dtype=torch.long)
        ...
        for batch_idx, entry in enumerate(batch):
            ...
            neg_ids[batch_idx, :entry['neg_length']] = entry['neg_ids']
            neg_length[batch_idx] = entry['neg_length']
        ...
        batch_entry['neg_length'] = neg_length
        return batch_entry
