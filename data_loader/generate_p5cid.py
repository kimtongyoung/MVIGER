...
########### data preprocessing ###########
with open(os.path.join(path, "sequential_data.txt"), "r") as f:
    data = f.read()

data = data.split("\n")[:-1]
items = sorted(set([int(a) for items in data for a in items.split(' ')[1:]]))
item_num = max(items) + 1
data = [d.split(" ")[1:-2] for d in data]

words = [[int(one_d) for one_d in d] for d in data]
...
adjacency_matrix = np.zeros((item_num, item_num))
...
########### save results here
# with open(os.path.join(path, "c{}_{}_CF_index.json".format(number_of_clusters, maximum_cluster_size)), "w") as f:
#     json.dump(item_CF_index_map, f)

##########################################################
###### apply indexing method 1 , described in paper ######
# with open(os.path.join(path, "c{}_{}_CF_index.json".format(number_of_clusters, maximum_cluster_size)), "r") as f:
#     data = json.load(f)
#
# ###### check repetition
# count = {}
...
#
# ###### save result
# with open(os.path.join(path, "computed_optimal_{}_CF_index.json".format(maximum_cluster_size)), "w") as f:
#     json.dump(final_data, f)

###################################################################################
###### apply indexing method 2, no repetition in cluster nodes and leaf node ######
# with open(os.path.join(path, "c{}_{}_CF_index.json".format(number_of_clusters, maximum_cluster_size)), "r") as f:
data = item_CF_index_map
...
for item, clusters in reformed_item_CF_index_map.items():
    if tuple(clusters) not in enumeration_by_group:
        full_index[item] = clusters + ["Leaf" + str(0)]
        enumeration_by_group[tuple(clusters)] = 1
        vocab += clusters + ["Leaf" + str(0)]
    else:
        full_index[item] = clusters + ["Leaf" + str(enumeration_by_group[tuple(clusters)])]

        vocab += clusters + ["Leaf" + str(enumeration_by_group[tuple(clusters)])]
        enumeration_by_group[tuple(clusters)] += 1

###### save result
with open(os.path.join(path, "sequential-data-p5cid.json".format(number_of_clusters, maximum_cluster_size)), "w") as f:
...
#
# ###### save result
# with open(os.path.join(path, "computed_no_repetition_{}_{}_CF_index.json".format(number_of_clusters, maximum_cluster_size)), "w") as f:
