...
class VQEmbedding(nn.Embedding):
    """VQ embedding mo  dule with ema update."""
...
class RQBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if not len(code_shape) == len(latent_shape) == 3:
            raise ValueError("incompatible split-gcn-code shape or latent shape")
    ...
    def embed_code_with_depth(self, code, to_latent_shape=False):
        '''
        do not reduce the split-gcn-code embedding over the axis of split-gcn-code-depth.
