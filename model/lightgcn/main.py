...
def save_embedding(root_path, emb_dict):
    with open(os.path.join(root_path, f'embedding768.pkl'), 'wb') as fOut:
        pickle.dump(emb_dict, fOut, protocol=pickle.HIGHEST_PROTOCOL)
    return print(f'saving  done!')


save_path = f'../../data/{world.dataset}/'
save_embedding(save_path, emb_dict)
print(f"Collaborative Embedding is saved at : {os.path.join('data', world.dataset, f'embedding768.pkl')}")