...
import numpy as np
import tqdm
from data_loader.indexing import *
...
def Amazon(dataset_name, rating_score):
    ...
    # for the before 2014 July data is used for previous research
    for inter in parse(data_file):
        # if int(inter['unixReviewTime']) >= 1406818800:
        #     continue
        # else:
...
def Amazon_meta(dataset_name, data_maps):
    datas = {}
    meta_file = f'data/amazon/raw/{dataset_name}/meta_{dataset_name}.json.gz'
...
def Yelp(dataset_name, date_min, date_max, rating_score):
    datas = []
    data_flie = f'data/amazon/raw/{dataset_name}/yelp_academic_dataset_review.json'
...
def Yelp_meta(dataset_name, datamaps):
    meta_infos = {}
    meta_file = f'data/amazon/raw/{dataset_name}/yelp_academic_dataset_business.json'
...
def main(data_name, rating_score, user_core, item_core):
    rating_score = rating_score  # rating score smaller than this score would be deleted
    ...
    # -------------- Save Data ---------------
    data_file = f'data/amazon/filtered/{data_name}/sequential_data.txt'
    meta_file = f'data/amazon/filtered/{data_name}/meta_data.json'
    datamaps_file = f'data/amazon/filtered/{data_name}/datamaps.json'
...
'''iid'''
# main('Beauty', 0, 5, 5)
# main('Sports_and_Outdoors', 0, 5, 5)
main('Yelp', 0, 5, 5)
###############################################################################
